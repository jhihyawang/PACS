...
def main(args):
    ...
    expected_feat_dim = args.feature_dim
    for k in ('head.0.weight', 'classifier.0.weight', 'mlp_head.0.weight'):
        if k in state_dict and state_dict[k].ndim == 2:
            expected_feat_dim = state_dict[k].shape[1]
            break
    # 建立模型架構
    ...
    # ).to(device)
    if args.early:
        # earlycat4：12通道 + 無 fusion（你的 Backbone 會自動把 3→12）
        backbone = MammographyCNNBackbone(
            backbone=args.backbone,
            feature_dim=expected_feat_dim,  # 多半是 2048；跟 ckpt 的 head 對齊
            pretrained=False,
            fusion_mode='early_cat4'
        )
        model = MammographyClassifier(
            backbone=backbone,
            num_classes=args.num_classes,
        ...
        # 你原本的 fusion 路徑（這裡示範：siamese 特徵 → 交給 fusion 的分類器）
        backbone = MammographyCNNBackbone(
            backbone=args.backbone,
            feature_dim=args.feature_dim,  # 走你原設定
            pretrained=False,
            fusion_mode='siamese'          # 輸出 (B,4,feat_dim)
        )
        # 若你用 ViT/Transformer 融合，這裡換成對應的分類器（你有引入 MammographyViT）
        # 例：
        # model = MammographyViT(backbone=backbone, num_classes=args.num_classes, ...)
        # 如果你現在的 MammographyClassifier 就是「不帶 fusion 的版本」，那就維持原寫法：
        model = MammographyClassifier(
            backbone=backbone,
            num_classes=args.num_classes,
            dropout=args.dropout
...
def parse_args():
    ...
    parser.add_argument('--num_classes', type=int, default=3,   # change
    ...
    parser.add_argument('--early', action='store_true', default=False,
                        help='Use early-cat-4 (12-ch) model without fusion')
