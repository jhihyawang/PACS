...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            improved = (val_loss < (prev_best))

            if improved:
                self.early_stop_wait = 0
            else:
                self.early_stop_wait += 1
                print(f"  [EarlyStopping] No improvement in val_loss for "
                    f"{self.early_stop_wait}/{self.early_stopping_patience} epochs")

            if self.early_stop_wait >= self.early_stopping_patience:
                print(f"\n[EarlyStopping] Stop training. val_loss has not improved for "
...
def parse_args():
    ...
    parser.add_argument('--img_size', type=int, default=512,     # change
                        help='Image size')
    
    # 模型相關
    parser.add_argument('--backbone', type=str, default='resnet50',
                        choices=['resnet50', 'efficientnet_b0', 'efficientnet_b3','densenet121'],
                        help='Backbone architecture')
    parser.add_argument('--feature_dim', type=int, default=2048,     # 原2048
                        help='Feature dimension')
    parser.add_argument('--num_classes', type=int, default=3,        # three class 0,1,2
                        help='Number of classes')
    parser.add_argument('--dropout', type=float, default=0.5,        # 原0.5
                        help='Dropout rate')
    ...
    parser.add_argument('--batch_size', type=int, default=8,         # change
                        help='Batch size')
    ...
    parser.add_argument('--early_stopping_patience', type=int, default=10, help='Number of epochs with no improvement to stop')
